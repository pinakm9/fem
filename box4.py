...
# Define mesh
mesh = dol.RectangleMesh(dol.Point(0,0), dol.Point(aspect_India, 1), 50, 50, "right/left")
#"""
dol.plot(mesh)
plt.show()
#"""
# Define function space
P1 = dol.FiniteElement('CG', dol.triangle, 4)
...
bcs = [dol.DirichletBC(FS, dol.Expression(('0','0', '0'), degree = 2), boundaries, 2),
       dol.DirichletBC(FS, dol.Expression(('-9.1*x[0]+5','5', '10'), degree = 2), boundaries, 4),
       dol.DirichletBC(FS, dol.Expression(("5*(1-x[1])", "5*(1-x[1])","10*(1-x[1])"), degree = 2), boundaries, 1),
       dol.DirichletBC(FS, dol.Expression(("-5*(1-x[1])", "5*(1-x[1])","10*(1-x[1])"), degree = 2), boundaries, 3)]
...
# Initial conditions
U_n = dol.interpolate(dol.Expression(("(-9.1*x[0]+5)*(1-x[1])", "5*(1-x[1])","10*(1-x[1])"), degree = 2), FS)
...
t = 0
pltr_T = plot.Plotter2(mesh, id_ = '2')
pltr_vel = plot.Plotter2(mesh, id_ = '2')
temp = lambda x,y: T_n([x,y])
vel = lambda x,y: (u_n([x,y]), v_n([x,y]))
for n in range(num_steps):
    t += dt
    if n%1 == 0:
        pltr_vel.stream(vel,'qtcm1/velocity/', n, t, quantity ='box4_vel')
        pltr_T.plot(temp,'qtcm1/velocity/', n, t, quantity ='box4_temp')
